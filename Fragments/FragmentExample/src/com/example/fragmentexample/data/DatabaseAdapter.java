...
public class DatabaseAdapter {
	...
	private Book extractBook(Cursor cursor) {
		...
		book.setPublishDate(new Date(cursor.getLong(cursor
				.getColumnIndex(BOOK_PUBLISH_DATE))));
	...
	class DatabaseOpenHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			for (int i = 0; i < 10; i++) {
				...
				cv.put(BOOK_PUBLISH_DATE, new Date().getTime());
				cv.put(BOOK_PUBLISHER, "Publisher" + i);
				cv.put(BOOK_ISBN, "ISBN" + i);
				db.insert(TBL_BOOKS, null, cv);
				cv.clear();
			}
